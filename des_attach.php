...
if (!empty($_GET['IDDmdePD'])){
    $IDDmdePD= $_GET['IDDmdePD'];
    $pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    
    $q = $pdo->prepare('SELECT *  FROM DemandePaiement where IDDmdePD = ?');
    $q->execute([$IDDmdePD]);
    $data = $q->fetch(PDO::FETCH_ASSOC);

    $nom_module = $data['Objet'];
...
$crud->setTable('attachement_decpt'); //definition de la table
$crud->setSubject('Attachement', 'Gestion des Attachement des Décomptes');

$crud->setRelation('IDDecpte', 'DemandePaiement', 'Objet');


//definition de l'afichage des champs de la table
$crud->displayAs(array(
	     	'IDDecpte' => "Demande Décompte",   
		'TravauxValPUFM' => 'Travaux sur Prix U/Forfaitaires Marché',
		'TravauxValPUFQ' => 'Travaux sur Prix U/Forfaitaires Calage des Qtés',
                'TravauxValPUFE' => "Travaux sur Prix U/Forfaitaires Executés",     
                'TaxeImpots' => "Taxes et Impôts Divers", 
                'PJ_Attachement' => "Pièce Jointe",            	
		)); 

		
		
$crud->columns(['IDDecpte','TravauxValPUFM','TravauxValPUFQ','TravauxValPUFE','TaxeImpots','PJ_Attachement']);
$crud->editFields(['IDDecpte','TravauxValPUFM','TravauxValPUFQ','TravauxValPUFE','TaxeImpots','PJ_Attachement']);
$crud->addFields(['TravauxValPUFM','TravauxValPUFQ','TravauxValPUFE','TaxeImpots','PJ_Attachement']);
$crud->readOnlyFields(['IDDecpte']);
...
$crud->callbackColumn('PJ_Attachement', function ($value, $row) {
    ...
    if (strlen($row->PJ_Attachement) != 0) { return '<div class="myGallery">
    <a href="/files/docprojet/'.$row->PJ_Attachement.'" download> <img src="/files/docprojet/telecharger.png" width="32" height="32"> </a>&nbsp;&nbsp;&nbsp;
    ​<span>'.$row->PJ_Attachement.'</span>
    </div>';}
           
});
$crud->where([
        '(IDDecpte = '.$IDDmdePD.' AND attachement_decpt.is_delete = 1)'
]);
...
$crud->callbackDelete(function ($stateParameters) {
    ...
    $pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $delete = $pdo->prepare('Update attachement_decpt SET is_delete = 0 WHERE IDattach = ?');
    $delete->execute([$id]);
...
$crud->callbackBeforeInsert(function ($stateParameters){
    ...
    $stateParameters->data['IDDecpte']=  $IDDmdePD;
...
if (isset($_SESSION['idu'])&& ( $_SESSION['droit']== "admin" || $_SESSION['droit'] == "courrier"  || $_SESSION['droit'] == "CSM" || $_SESSION['droit'] == "UGAD")){ // si paramètres correct. 
?>
