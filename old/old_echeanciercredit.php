...
<body id="page-top" data-bs-spy="scroll" data-bs-target="#mainNav" data-bs-offset="54">
    ...
    <section id="services" style="padding: 0px 0px;padding-top: 0px;padding-left: 0px;padding-bottom: 0px;">
        ...
        <section class="mt-4" style="padding: 3px 0px;">
	        ...
	        echo "  <thead class='thead-dark'><tr class='table-primary'> <td> No FACTURE </td> <td> Client </td> <td> Montant Facture </td> <td> Date de facture </td>  <td> Déja payé </td> <td> Reste à payer </td> <td> Dû à ce jour (En retard de paiement) </td> </thead> <tbody>";
	        
	                               foreach($results as $row){
	                               
	                                 //recupère somme des debits du mois
					 $sql1 = "SELECT  sum(MONTANT) as TOTALPAYE FROM OPERATION_CREDIT  where IDCREA = ?   ";
				         $q1 = $pdo->prepare($sql1);
			       		 $q1->execute(array($row['IDCREA']));
					 ...
					 $total_paye = $data1['TOTALPAYE']; //total déja payé pour une facture...
	                                ...
	                                echo "<tr> <td> ".$row['NUMERO_FACTURE']." </td> <td>".get_client($pdo,$row['IDC'])." </td> <td>-".number_format($row['MONTANT'], 0, '.', ' ')." </td> <td>".$row['DATE_FACTURATION']."</td>  <td>-".number_format($total_paye, 0, '.', ' ')."</td><td>".number_format( $total_rest, 0, '.', ' ')."</td> <td>".number_format($total_rest, 0, '.', ' ')." XAF</td>  </tr>";
