...
$crud->setTable('Avenant'); //definition de la table
$crud->setSubject('Avenant', 'Gestion des Avenants ');

$crud->setRelation('IDContrat', 'Contrats', 'Objet_contrat');
...
$crud->displayAs(array(
	     	'IDContrat' => "Contrat",   
		'Motif' => 'Description',
		'Montant' => 'Montant de L\'avenant',
                'PJ_New_OS' => "Nouvel Ordre de Service",  
                'date' => "Date",               	
		)); 

		
		
 $crud->columns(['IDContrat','Motif','Montant','date','PJ_New_OS']);
 $crud->editFields(['IDContrat','Motif','Montant','date','PJ_New_OS']);
 $crud->addFields(['IDContrat','Motif','Montant','date','PJ_New_OS']);
$crud->requiredFields(['IDContrat','date']);
...
$crud->callbackColumn('PJ_New_OS', function ($value, $row) {
    //return "<div style='text-align : left;'>". wordwrap($row->besoin_marche, 50, "<br>", true)."</div>"; 
    if (strlen($row->PJ_New_OS) != 0) { return '<div class="myGallery">
    <a href="/files/docprojet/'.$row->PJ_New_OS.'" download> <img src="/files/docprojet/telecharger.png" width="32" height="32"> </a>&nbsp;&nbsp;&nbsp;
    ​<span>'.$row->PJ_New_OS.'</span>
    </div>';}
...
$crud->callbackDelete(function ($stateParameters) {
    ...
    $delete = $pdo->prepare('Update Avenant SET is_delete = 0 WHERE IDAvenant = ?');
    $delete->execute([$id]);
