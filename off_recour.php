...
$crud->displayAs(array(
		...
		'Type' => 'Type d\'offre',
		'IDProjet' => "Projet", 
		'objet_recours' => 'Objet du Recours',
                'IDSoumiss' => "Soumissionaire",   
                'date_recours' => "Date",    
                'PJ_recours' => "Pièce Jointe du Recours",            	
		)); 

		
		
 $crud->columns(['IDAvis','IDSoumiss','IDProjet','objet_recours','date_recours','PJ_recours']);
 $crud->editFields(['IDAvis','IDSoumiss','objet_recours','date_recours','PJ_recours']);
 $crud->addFields(['IDAvis','IDSoumiss','objet_recours','date_recours','PJ_recours']);
$crud->requiredFields(['IDAvis', 'objet_recours','date_recours']);
...
$crud->callbackColumn('objet_recours', function ($value, $row) {
    return "<div>" . wordwrap($row->objet_recours, 50, "<br>", false) . "</div>";
});

$crud->callbackColumn('PJ_recours', function ($value, $row) {
    //return "<div style='text-align : left;'>". wordwrap($row->besoin_marche, 50, "<br>", true)."</div>"; 
    if (strlen($row->PJ_recours) != 0) { return '<div class="myGallery">
    <a href="/files/docprojet/'.$row->PJ_recours.'" download> <img src="/files/docprojet/telecharger.png" width="32" height="32"> </a>&nbsp;&nbsp;&nbsp;
    ​<span>'.$row->PJ_recours.'</span>
    </div>';}
...
$crud->callbackBeforeInsert(function ($stateParameters){
    ...
    $stateParameters->data['is_recours']=  0;
