...
$crud->setSubject('Evaluation', 'Gestion des Evaluationdes Offres ');
...
$crud->displayAs(array(
		...
		'objet_offre' => 'Objet de l\'offre',
                ...
                'PJ_RapportNot' => "Rapport d'evaluation",          	
		)); 

		
		
$crud->columns(['IDAvis','Type','objet_offre','date_evaluation','IDSoumiss','Note','Commentaire','PJ_RapportNot']);
$crud->editFields(['IDAvis','Type','objet_offre','date_evaluation','IDSoumiss','Note','Commentaire','PJ_RapportNot']);
$crud->addFields(['IDAvis','Type','objet_offre','date_evaluation','IDSoumiss','Note','Commentaire','PJ_RapportNot']);
$crud->requiredFields(['Note','date_evaluation']);
$crud->readOnlyFields(['IDAvis','Type','objet_offre','IDSoumiss']);
...
$crud->callbackColumn('Commentaire', function ($value, $row) {
    return "<div>" . wordwrap($row->Commentaire, 50, "<br>", false) . "</div>";
});

$crud->callbackColumn('PJ_RapportNot', function ($value, $row) {
    //return "<div style='text-align : left;'>". wordwrap($row->besoin_marche, 50, "<br>", true)."</div>"; 
    if (strlen($row->PJ_RapportNot) != 0) { return '<div class="myGallery">
    <a href="/files/docprojet/'.$row->PJ_RapportNot.'" download> <img src="/files/docprojet/telecharger.png" width="32" height="32"> </a>&nbsp;&nbsp;&nbsp;
    ​<span>'.$row->PJ_RapportNot.'</span>
    </div>';}
