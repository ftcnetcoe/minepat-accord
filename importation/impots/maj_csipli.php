...
$inputFileName = 'Gestion des operations de débit_2023-02-03 Impots.xlsx';
...
$i++;
    ...
    $LibelleOp = "Achat"; //
	 ...
	 $requete->execute(array($DateOP,"ACHAT",$MOTIF ,"",$MONTANT,15,6,$IDPROJ,$CLIENT_FOURNISS,$i,getperiod($DateOP),$lettrage,$op,"IMPOTS")); //1 représente journal de vente
	 
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($DateOP,"ACHAT",$MOTIF ,$MONTANT,"",235,6,$IDPROJ,$CLIENT_FOURNISS,$i,getperiod($DateOP),$lettrage,$op,"IMPOTS")); //1 représente journal de vente
	 ...
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($DateOP,"ACHAT",$MOTIF ,"",$MONTANT,$cpt,$jnal,$IDPROJ,$CLIENT_FOURNISS,$i,getperiod($DateOP),$lettrage,$op,"IMPOTS")); //1 représente journal de vente
	 
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($DateOP,"ACHAT",$MOTIF ,$MONTANT,"",15,$jnal,$IDPROJ,$CLIENT_FOURNISS,$i,getperiod($DateOP),$lettrage,$op,"IMPOTS")); //1 représente journal de vente
