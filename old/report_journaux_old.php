...
<H1 style="color:#3358FF;">LIVRE DES COMPTES DU JOURNAL '.$_POST['journal'].'</H1></caption>
...
<thead BGCOLOR="red">
   ...
   $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	...
	$q4 = $pdo->prepare("SELECT * FROM `ca_journal` WHERE CodeJnal = '$type_jour' ");
	$q4->execute();
...
while($data4= $q4->fetch(PDO::FETCH_ASSOC)){
	...
	<th colspan="3"> Total JOURNAL '.$data4['CodeJnal'].'- '.$data4['Libelle'].' </th>
	<th> du '.$date_debut.'</th>
	<th> au '.$date_fin.'</th>
	<th align = "right"> </th>
	<th align = "right"> </th>
    ...
    <tbody>
		...
		<td>'.$data3['CodeOp'].'</td>
	        ...
	        <td>'.number_format($data3['MontantOpDebit'], 0, '.', ' ').'</td>
		<td>'.number_format($data3['MontantOpCredit'], 0, '.', ' ').'</td>
	...
	header('Location: /rh.kiama.cm/recherche_journal.php' );  
}