...
$crud->setSubject('OS', 'Gestion des OS de Contrat');
...
$crud->displayAs(array(
		...
		'Reference_os' => 'Réference',
                'date_signature' => "Date de Signature",
                'Objet_contrat' => "Objet",     
                'date_dem_os' => "Date de Démarrage",
                'PJ_OS' => "Ordre de Service",                	
		)); 

		
		
 $crud->columns(['IDProjet','IDSoumiss','Objet_contrat','Reference_os','date_signature','date_dem_os','PJ_OS']);
 $crud->editFields(['IDProjet','IDSoumiss','Objet_contrat','Reference_os','date_signature','date_dem_os','PJ_OS']);
 $crud->addFields(['IDProjet','IDSoumiss','Objet_contrat','Reference_os','date_signature','date_dem_os','PJ_OS']);
$crud->requiredFields(['Reference_os','date_signature','date_dem_os','PJ_OS']);
...
$crud->callbackColumn('PJ_OS', function ($value, $row) {
    //return "<div style='text-align : left;'>". wordwrap($row->besoin_marche, 50, "<br>", true)."</div>"; 
    if (strlen($row->PJ_OS) != 0) { return '<div class="myGallery">
    <a href="/files/docprojet/'.$row->PJ_OS.'" download> <img src="/files/docprojet/telecharger.png" width="32" height="32"> </a>&nbsp;&nbsp;&nbsp;
    ​<span>'.$row->PJ_OS.'</span>
    </div>';}
...
$crud->callbackBeforeInsert(function ($stateParameters){
    ...
    $projet = $stateParameters->data['IDProjet'];
    
    
    $stateParameters->data['date_creation']=  $dateact;
    $stateParameters->data['id_creation']=  $_SESSION['idu'];
    
    $sql = "SELECT Auteur FROM Projet WHERE (YEAR(date_creation) = $annee && is_delete = 1)";
    $nbr2 = $pdo->query($sql);
    $soumiss = $nbr2->fetchColumn();
    $stateParameters->data['IDSoumiss']=  $soumiss;
