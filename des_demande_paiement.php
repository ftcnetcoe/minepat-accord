...
$crud->displayAs(array(
		...
		'resume' => 'Description',
                'date_demande' => "Date", 
                'PJ_demande' => "Pièce Jointe",
                'PJ_demande_autre' => "Autres Document",
                'PJ_Attachement' => "Attachement Complet",               	
		)); 

		
		
$crud->columns(['PJ_Attachement','IDProjet','IDSoumiss','date_demande','Objet','resume','PJ_demande','PJ_demande_autre']);
$crud->editFields(['IDProjet','IDSoumiss','date_demande','Objet','resume','PJ_demande','PJ_demande_autre']);
$crud->addFields(['IDProjet','date_demande','Objet','resume','PJ_demande','PJ_demande_autre']);
$crud->requiredFields(['IDProjet','Objet','date_demande','PJ_demande']);
$crud->readOnlyFields(['IDSoumiss']);

$crud->where([
    'DemandePaiement.is_delete = 1',
]);
$crud->callbackColumn('PJ_demande', function ($value, $row) {
    //return "<div style='text-align : left;'>". wordwrap($row->besoin_marche, 50, "<br>", true)."</div>"; 
    if (strlen($row->PJ_demande) != 0) { return '<div class="myGallery">
    <a href="/files/docprojet/'.$row->PJ_demande.'" download> <img src="/files/docprojet/telecharger.png" width="32" height="32"> </a>&nbsp;&nbsp;&nbsp;
    ​<span>'.$row->PJ_demande.'</span>
    </div>';}
           
});$crud->callbackColumn('PJ_demande_autre', function ($value, $row) {
    //return "<div style='text-align : left;'>". wordwrap($row->besoin_marche, 50, "<br>", true)."</div>"; 
    if (strlen($row->PJ_demande_autre) != 0) { return '<div class="myGallery">
    <a href="/files/docprojet/'.$row->PJ_demande_autre.'" download> <img src="/files/docprojet/telecharger.png" width="32" height="32"> </a>&nbsp;&nbsp;&nbsp;
    ​<span>'.$row->PJ_demande_autre.'</span>
    </div>';}
...
$crud->callbackColumn('Objet', function ($value, $row) {
 ...
 $crud->callbackColumn('PJ_Attachement', function ($value, $row) {

            $modal='<center><div class="modal" id="exampleModal'.$row->IDDmdePD.'" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-fullscreen-xl-down" style="width: 1240px" role="document">
              <div class="modal-content" style="min-height: 600px; max-height: 900px;">
                ...
                <div class="modal-body">
                    <iframe src="http://minepat.kiama.cm/des_attach.php?IDDmdePD='.$row->IDDmdePD.'" frameborder="0" style="overflow:hidden;overflow-x:hidden;overflow-y:hidden;height:100%;width:100%;position:absolute;top:0%;left:0px;right:0px;bottom:0px" height="100%" width="100%"></iframe>
                </div>
           ...
           $modal= $modal. '<button type="button" class="btn btn-default" data-toggle="modal" data-target="#exampleModal'.$row->IDDmdePD.'">Attachement</button>';
...
$crud->callbackDelete(function ($stateParameters) {
    ...
    $delete = $pdo->prepare('Update DemandePaiement SET is_delete = 0 WHERE IDDmdePD = ?');
    $delete->execute([$id]);
...
$crud->callbackBeforeInsert(function ($stateParameters){
    ...
    $q3 = $pdo->prepare('SELECT * FROM Contrats where IDContrats = ?');
        $q3->execute([$stateParameters->data['IDProjet']]);
        $data = $q3->fetch(PDO::FETCH_ASSOC);
        //$IDProjet = $data['IDProjet'];
        $IDSoumiss = $data['IDSoumiss'];
        
        //$stateParameters->data['IDProjet']=  $IDProjet;
    	$stateParameters->data['IDSoumiss']=  $IDSoumiss;
...
if (isset($_SESSION['idu'])&& ( $_SESSION['droit']== "admin" || $_SESSION['droit'] == "courrier" || $_SESSION['droit'] == "CSM" || $_SESSION['droit'] == "UGAD")){ 
?>
<html>
...
<head>
        ...
        <!-- ======= Template Main CSS File Dashboard ======= -->
                <?php if ($_SESSION['droit'] != "admin" && $_SESSION['droit']!= "courrier" && $_SESSION['droit']!= "CSM" && $_SESSION['droit'] != "UGAD") {
