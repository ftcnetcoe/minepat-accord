...
$inputFileName = 'credits_emprunts.xls';
...
$i=1345; // a modifier demain

$op=692; //à modifier au suivant
...
for ($row = 2; $row <= $highestRow; $row++){ 
    ...
    */
	...
	// insertion insertion ligne
	 ...
	 $requete->execute(array($DateOP,"","Emprunt" ,"",$MONTANT,4,3,"","",$i,getperiod ($DateOP),$lettrage,$op,"EMPRUNT")); //1 représente journal de vente
	 
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($DateOP,"","Emprunt" ,$MONTANT,"",38,3,"","",$i,getperiod ($DateOP),$lettrage,$op,"EMPRUNT")); //1 représente journal de vente
