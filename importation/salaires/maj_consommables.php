...
$inputFileName = 'Gestion des operations de débit_2023-02-03 CNPS.xlsx';
...
//$i++;
   ...
   // $DateOP = date('Y-m-d', $ligne[0][2]);
	...
	/*
	 ...
	 $requete->execute(array($DateOP,"SALAIRE",$MOTIF ,"",$MONTANT,220,6,$IDPROJ,$CLIENT_FOURNISS,$i,getperiod($DateOP),$lettrage,$op,"SALAIRE_NET")); //1 représente journal de vente
	 
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($DateOP,"SALAIRE",$MOTIF ,$MONTANT,"",216,6,$IDPROJ,$CLIENT_FOURNISS,$i,getperiod($DateOP),$lettrage,$op,"SALAIRE_NET")); //1 représente journal de vente
	 */
	 ...
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($DateOP,"SALAIRE",$MOTIF ,"",$MONTANT,$cpt,$jnal,$IDPROJ,$CLIENT_FOURNISS,$i,getperiod($DateOP),$lettrage,$op,"SALAIRE_CNPS")); //1 représente journal de vente
	 
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($DateOP,"SALAIRE",$MOTIF ,$MONTANT,"",217,$jnal,$IDPROJ,$CLIENT_FOURNISS,$i,getperiod($DateOP),$lettrage,$op,"SALAIRE_CNPS")); //1 représente journal de vente
