...
$inputFileName = 'credits_clients.xls';
...
for ($row = 2; $row <= $highestRow; $row++){ 
    ...
    $date = date('Y-m-d', PHPExcel_Shared_Date::ExcelToPHP($ligne[0][3]));
    $datefacturation = date('Y-m-d', PHPExcel_Shared_Date::ExcelToPHP($ligne[0][4]));  
    $projet = $ligne[0][5];
    $creance= $ligne[0][6];
    ...
    */
	...
	//TTC
	 ...
	 $requete->execute(array($datefacturation,$creance,$Motif ,$TTC,"",212,1,$projet,"",$i,getperiod ($datefacturation),$lettrage,$op,"SPI")); //1 représente journal de vente
	...
	if ($TVA !=0){
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($datefacturation,$creance,$Motif ,"",$TVA,31,1,$projet,"",$i,getperiod ($datefacturation),"",$op,"SPI")); //1 représente journal de vente
	}
	
	//HT
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($datefacturation,$creance,$Motif ,"",$HT,28,1,$projet,"",$i,getperiod($datefacturation),"",$op,"SPI")); //1 représente journal de vente
	...
	//TTC (Modifier journal = 1  en fonction CAISSE ou AFB
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($datefacturation,$creance,$Motif ,"",$TTC,212,$jnal,$projet,"",$i,getperiod ($datefacturation),$lettrage,$op,"SPI")); //1 représente journal de vente
	
	if ($TVA !=0){
	//TVA RS (Modifier journal = 1  en fonction CAISSE ou AFB
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($datefacturation,$creance,$Motif ,$TVA,"",25,$jnal,$projet,"",$i,getperiod ($datefacturation),"",$op,"SPI")); //1 représente journal de vente
	}
	
	 //IR RS (Modifier journal = 1  en fonction CAISSE ou AFB
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($datefacturation,$creance,$Motif ,$IR,"",26,$jnal,$projet,"",$i,getperiod ($datefacturation),"",$op,"SPI")); //1 représente journal de vente
	 ...
	 $requete = $pdo->prepare("INSERT INTO ca_operation (DateOp,RefPj,LibelleOp,MontantOpDebit,MontantOpCredit,Numcpt,idJnal,IDPROJ,CLIENT_FOURNISS,NUM,Period,lettrage,op,ancientypeop) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	 $requete->execute(array($datefacturation,$creance,$Motif ,$MONTANTNAP,"",$comptea,$jnal,$projet,"",$i,getperiod ($datefacturation),"",$op,"SPI")); //1 représente journal de vente
