...
$crud->setTable('AvisAppelOffre'); //definition de la table
$crud->setSubject('Avis', 'Gestion des Avis d\'appel des Offres ');
...
$crud->displayAs(array(
	     	'IDProjet' => "Projet",   
		'MO_AC' => 'MO MOD',
		'objet_avis' => 'Objet de l\'avis',
                'date' => "Date",  
                'date_publication' => "Date de publication", 
                'PJ_DAO' => "Pièce Jointe de l'avis",               	
		)); 

		
		
 $crud->columns(['IDProjet','MO_AC','date','objet_avis','date_publication','PJ_DAO']);
 $crud->editFields(['IDProjet','MO_AC','date','objet_avis','date_publication','PJ_DAO']);
 $crud->addFields(['IDProjet','MO_AC','date','objet_avis','date_publication','PJ_DAO']);
$crud->requiredFields(['IDProjet','date', 'objet_avis']);

$crud->where([
    'AvisAppelOffre.is_delete = 1'
]);

$crud->callbackColumn('objet_avis', function ($value, $row) {
    return "<div>" . wordwrap($row->objet_avis, 50, "<br>", false) . "</div>";
});

$crud->callbackColumn('PJ_DAO', function ($value, $row) {
    //return "<div style='text-align : left;'>". wordwrap($row->besoin_marche, 50, "<br>", true)."</div>"; 
    if (strlen($row->PJ_DAO) != 0) { return '<div class="myGallery">
    <a href="/files/docprojet/'.$row->PJ_DAO.'" download> <img src="/files/docprojet/telecharger.png" width="32" height="32"> </a>&nbsp;&nbsp;&nbsp;
    ​<span>'.$row->PJ_DAO.'</span>
    </div>';}
...
$crud->callbackDelete(function ($stateParameters) {
    ...
    $delete = $pdo->prepare('Update AvisAppelOffre SET is_delete = 0 WHERE IDAvis = ?');
    $delete->execute([$id]);
