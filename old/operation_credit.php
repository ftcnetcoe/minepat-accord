...
$crud->setTable('OPERATION_CREDIT');
$crud->setSubject('Operation de credit', 'Gestion des operations de crédit');

$crud->setRelation('IDCOMPTEOP', 'COMPTE_OPERATION', 'NOMCOMPTE');
$crud->setRelation('IDPROJ', 'PROJET', 'NOMPROJ');

$crud->setRelation('IDCREA', 'CREANCES', '{NUMERO_FACTURE}-{LIBELLE}');


$crud->setRelation('IDCOMPTEC', 'COMPTE_CREDIT', 'NOMCOMPTE');
$crud->displayAs(array(
	    ...
	    'IDPROJ' => 'Projet',
			 'IDCREA' => 'CREANCE',
			 'IDCOMPTEC' => 'Compte créditeur',
		)); 

$crud->columns(['IDOPERATION', 'IDCOMPTEOP', 'IDCOMPTEC', 'DATEOP','IDPROJ' , 'IDCREA', 'MONTANT','MOTIF']);
...
//$crud-> where("month(DATEOP)=".$mois.""); // on filtre par mois ici 
...
$crud->unsetDeleteMultiple();
 ...
 $crud->callbackBeforeInsert(function ($stateParameters) {
  ...
  $stateParameters->data['MONTANT']=preg_replace('/[^0-9]/', '',$stateParameters->data['MONTANT']);
	...
	//additione au montant du formulaire
	...
	$sql = "SELECT SOLDE  FROM COMPTE_CREDIT where IDCOMPTEC = ? ";
	$q = $pdo->prepare($sql);
    $q->execute([$stateParameters->data['IDCOMPTEC']]);
	$data= $q->fetch(PDO::FETCH_ASSOC);
	$solde= $data['SOLDE'];
	
	//je soustrait
	$newsolde_credit = $solde-$stateParameters->data['MONTANT'];
	
	//met a jours solde associé
    $sql = "update COMPTE_CREDIT set SOLDE = ?  where IDCOMPTEC = ? ";
	$q = $pdo->prepare($sql);
    $q->execute([$newsolde_credit,$stateParameters->data['IDCOMPTEC']]);
	
	
	 // sauvegarde le solde ************************************************************************
     /* $sql = "INSERT INTO SAUVEGARDE_SOLDE (IDCOMPTEOP, SOLDE, MOTIF) VALUES (?,?,?)";
      $stmt= $pdo->prepare($sql);
      $stmt->execute([$stateParameters->data['IDCOMPTEOP'],$newsolde_credit,'[INSERT_CREDIT]'.$stateParameters->data['MOTIF'] ]);
      */
...
/*
//en cas de mise à jour ( à revoir)
$crud->callbackBeforeDelete(function ($stateParameters) {
	//je recupère le solde de l'operation
	//j'ajoute le montant  au compte debiteur
	//je retranche au compte associé
	
	 $pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	
	$sql = "SELECT IDCOMPTEOP  FROM OPERATION_DEBIT where IDOPERATION = ? ";
	$q = $pdo->prepare($sql);
    $q->execute([$stateParameters->primaryKeyValue]);
	$data= $q->fetch(PDO::FETCH_ASSOC);
	$IDCOMPTEOP = $data['IDCOMPTEOP'];
	
	//recupère solde compte operation
	$sql = "SELECT SOLDE  FROM COMPTE_OPERATION where IDCOMPTEOP = ? ";
	$q = $pdo->prepare($sql);
    $q->execute([$IDCOMPTEOP]);
	$data= $q->fetch(PDO::FETCH_ASSOC);
	$solde= $data['SOLDE'];
	...
	$sql = "update COMPTE_OPERATION set SOLDE = ?  where IDCOMPTEOP = ? ";
	$q = $pdo->prepare($sql);
    $q->execute([$newsolde,$IDCOMPTEOP]);
	
	
	$sql = "SELECT IDCOMPTED  FROM OPERATION_DEBIT where IDOPERATION = ? ";
	$q = $pdo->prepare($sql);
    $q->execute([$stateParameters->primaryKeyValue]);
	$data= $q->fetch(PDO::FETCH_ASSOC);
	$IDCOMPTED = $data['IDCOMPTED'];
	...
	$q = $pdo->prepare($sql);
    $q->execute([$newsolde_debit,$IDCOMPTED]);
...
$crud->callbackBeforeUpdate(function ($stateParameters){
 ...
 $stateParameters->data['MONTANT']=preg_replace('/[^0-9]/', '',$stateParameters->data['MONTANT']);
	...
	$sql = "SELECT MONTANT FROM OPERATION_CREDIT where IDOPERATION = ? ";
	$q = $pdo->prepare($sql);
        $q->execute([$stateParameters->primaryKeyValue]);
	$data= $q->fetch(PDO::FETCH_ASSOC);
   ...
   /*   $sql = "INSERT INTO SAUVEGARDE_SOLDE (IDCOMPTEOP, SOLDE, MOTIF) VALUES (?,?,?)";
      $stmt= $pdo->prepare($sql);
      $stmt->execute([$stateParameters->data['IDCOMPTEOP'],$newsolde,'[UPDATE_CREDIT]'.$stateParameters->data['MOTIF'] ]);
...
<head>
    ...
    function doReload(mois){ 
		...
		document.location.href = "operation_credit.php?mois="+mois;	
    }
...
<div class="row">
	...
	<?php						
	 ...
	 include_once("solde_compte_operations.php");
	  echo "<a href='operation_credit.php' class='btn btn-primary'> ACTUALISER </a>";
	  echo "</center>";
	?>
